...
class TestNerClient(unittest.TestCase):
    ...
    def test_gets_ents_returns_dictionary_given_nonempty_string_causes_empty_spacy_doc_ents(self):
        model = NerModelTestDouble("eng")
        model.returns_doc_ents([])
        ner = NamedEntityClient(model)
        ents = ner.get_ents("Madison is a city in Wiscosin")
        self.assertIsInstance(ents, dict)
    ...
    def test_get_ents_given_spacy_GPE_is_returned_serializes_to_Language(self):
        model = NerModelTestDouble("eng")
